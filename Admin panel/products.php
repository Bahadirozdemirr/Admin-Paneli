					...
					<div class="card-body">
							...
							<div class="form-group">
								<div class="custom-control custom-switch">
								  ...
								  <label class="custom-control-label" for="status">Mevcut</label>
								</div>
							</div>
					</div>
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="button" onclick="$('#manage-product').get(0).reset()">İptal</button>
							</div>
						</div>
					</div>
				</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										...
										<p><small>Fiyat: <b><?php echo number_format($row['price'],2) ?></b></small></p>
										<p><small>Durum: <b><?php echo $row['status'] == 1 ? " Available" : "Unavailable" ?></b></small></p>
										<p><small>Tanım: <b><?php echo $row['description'] ?></b></small></p>
									</td>
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_product" type="button" data-id="<?php echo $row['id'] ?>" data-description="<?php echo $row['description'] ?>" data-name="<?php echo $row['name'] ?>"  data-price="<?php echo $row['price'] ?>"  data-status="<?php echo $row['status'] ?>" data-category_id="<?php echo $row['category_id'] ?>">Düzenle</button>
										<button class="btn btn-sm btn-danger delete_product" type="button" data-id="<?php echo $row['id'] ?>">Sil</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('.delete_product').click(function(){
		_conf("","delete_product",[$(this).attr('data-id')])
	})
